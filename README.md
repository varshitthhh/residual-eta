...
simple baseline guess (a historical average, or distance divided by average speed) and
corrects it -- the same design Uber's production ETA system (DeepETA) uses. Rather than
a single number, each prediction comes with a "likely" estimate and a calibrated "safe,
...
- **Per-leg asymmetric loss chosen by evidence, not assumed.** Quantile regression's
  alpha parameter *is* the business cost ratio between early and late errors. Testing
...
- **Measured, not assumed, latency and throughput.** Found and fixed a real performance
...
NYC TLC High-Volume For-Hire Services (HVFHS) trip records -- real, publicly released
Uber/Lyft/Via/Juno trip data. https://www.nyc.gov/site/tlc/about/tlc-trip-record-data.page

- **Period:** February, June, October 2024 (seasonal spread, about 54.7M rows after
  cleaning from 59.5M raw -- 91.9% retention)
- **Split:** time-based, not random -- Feb to train, Jun to validation, Oct to test
...
Raw HVFHS Parquet (DuckDB, out-of-core)
  ...
  -> Baselines: dispatch = zone x hour historical avg (borough -> citywide fallback);
     trip = trip_miles / avg speed-by-hour
  -> Feature engineering (effect-size validated, not assumed) + residual targets
  -> LightGBM x 4 (dispatch/trip x point/safe quantile models)
...
**Latency and throughput** (local benchmark, 20-core machine):
- Unloaded: p50 about 52ms, p99 about 90ms
- Single-worker capacity ceiling: about 23-25 req/sec (flat regardless of concurrency)
- 4-worker capacity ceiling: about 78-79 req/sec (about 3.3x scaling -- sub-linear due
  to shared resource overhead, not a bug)
...
Python 3.9+ (uses typing.Optional, not the pipe union syntax, for broader compatibility).

## Project structure

rideshare-eta/
- models/        trained LightGBM models, calibration, category encodings
- lookups/        baseline and feature lookup tables (zone x hour, speed, tolls)
